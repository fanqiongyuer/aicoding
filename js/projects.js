
// 作品数据
const projects = [
    {
        id: 1,
        title: "移动端应用UI设计",
        description: "这是一个专为健身爱好者设计的移动应用UI。项目包括用户界面设计、交互设计和原型制作。主要特点包括简洁的设计风格、易于导航的界面和突出健身数据的可视化展示。",
        image: "https://picsum.photos/id/1/600/400",
        category: "ui",
        link: "#",
        detailImages: [
            "https://picsum.photos/id/1/800/600",
            "https://picsum.photos/id/10/800/600",
            "https://picsum.photos/id/100/800/600"
        ]
    },
    {
        id: 2,
        title: "品牌标识设计",
        description: "为一家新创科技公司设计的品牌标识。设计过程包括市场研究、竞品分析和多轮创意草图。最终设计体现了公司的创新精神和科技感，同时保持了简约和易识别的特点。",
        image: "https://picsum.photos/id/20/600/400",
        category: "branding",
        link: "#",
        detailImages: [
            "https://picsum.photos/id/20/800/600",
            "https://picsum.photos/id/21/800/600",
            "https://picsum.photos/id/22/800/600"
        ]
    },
    {
        id: 3,
        title: "产品包装设计",
        description: "为一系列有机食品设计的包装。设计目标是突出产品的天然和环保特性，同时吸引目标消费群体的注意。设计采用可持续材料，使用清新的色彩和简洁的图形元素。",
        image: "https://picsum.photos/id/30/600/400",
        category: "graphic",
        link: "#",
        detailImages: [
            "https://picsum.photos/id/30/800/600",
            "https://picsum.photos/id/31/800/600",
            "https://picsum.photos/id/32/800/600"
        ]
    },
    {
        id: 4,
        title: "电商网站设计",
        description: "为服装电商平台设计的网站界面。项目范围包括首页、产品列表页、产品详情页和结账流程。设计强调良好的用户体验，清晰的视觉层次和响应式布局。",
        image: "https://picsum.photos/id/40/600/400",
        category: "ui",
        link: "#",
        detailImages: [
            "https://picsum.photos/id/40/800/600",
            "https://picsum.photos/id/41/800/600",
            "https://picsum.photos/id/42/800/600"
        ]
    },
    {
        id: 5,
        title: "企业宣传册设计",
        description: "为咨询公司设计的企业宣传册。设计需要展示公司的专业形象和服务领域，同时保持良好的可读性和吸引力。采用了现代排版技术和高质量的图片素材。",
        image: "https://picsum.photos/id/50/600/400",
        category: "graphic",
        link: "#",
        detailImages: [
            "https://picsum.photos/id/50/800/600",
            "https://picsum.photos/id/51/800/600",
            "https://picsum.photos/id/52/800/600"
        ]
    },
    {
        id: 6,
        title: "公司形象设计",
        description: "为一家金融科技公司进行的品牌形象重新设计。项目包括标志、名片、信纸、信封等企业基础应用系统的设计。设计强调专业、可靠和创新的品牌形象。",
        image: "https://picsum.photos/id/60/600/400",
        category: "branding",
        link: "#",
        detailImages: [
            "https://picsum.photos/id/60/800/600",
            "https://picsum.photos/id/61/800/600",
            "https://picsum.photos/id/62/800/600"
        ]
    },
    {
        id: 7,
        title: "活动海报设计",
        description: "为音乐节设计的系列海报。设计需要吸引年轻观众，同时清晰传达活动信息。采用大胆的色彩和独特的图形元素，创造出充满活力的视觉效果。",
        image: "https://picsum.photos/id/70/600/400",
        category: "graphic",
        link: "#",
        detailImages: [
            "https://picsum.photos/id/70/800/600",
            "https://picsum.photos/id/71/800/600",
            "https://picsum.photos/id/72/800/600"
        ]
    },
    {
        id: 8,
        title: "智能家居应用设计",
        description: "为智能家居系统设计的控制应用界面。设计需要简化复杂的控制功能，提供直观的用户界面。界面设计采用了深色主题，配合明亮的控制元素。",
        image: "https://picsum.photos/id/80/600/400",
        category: "ui",
        link: "#",
        detailImages: [
            "https://picsum.photos/id/80/800/600",
            "https://picsum.photos/id/81/800/600",
            "https://picsum.photos/id/82/800/600"
        ]
    },
    {
        id: 9,
        title: "咖啡品牌设计",
        description: "为精品咖啡店设计的品牌形象。设计包括标志、菜单、包装和店面视觉元素。设计风格结合了复古和现代元素，强调咖啡的手工品质和独特风味。",
        image: "https://picsum.photos/id/90/600/400",
        category: "branding",
        link: "#",
        detailImages: [
            "https://picsum.photos/id/90/800/600",
            "https://picsum.photos/id/91/800/600",
            "https://picsum.photos/id/92/800/600"
        ]
    }
]; 
